...
import type { GameState, MatchRules } from '$lib/engine/types';
...
describe('codec', () => {
  it('round-trips states in every phase exactly', () => {
    ...
    let s: GameState = createState(4, rules);
    const seen = new Set<string>();
    const check = () => {
      seen.add(s.match.winner ? 'winner' : s.match.phase.kind);
      expect(decodeState(encodeState(s), rules)).toEqual(s);
    };
...
describe('LocalSource', () => {
  it('records inputs that replay to its latest state', async () => {
    ...
    const source = new LocalSource([controller, controller], { kind: 'time', minutes: 1 });
